namespace ChronoTrigger
{
	internal class SaveData
	{
		...
		private readonly System.Text.Encoding mEncode;
		public uint Adventure { private get; set; } = 0;

		private SaveData()
		{
			System.Text.Encoding.RegisterProvider(System.Text.CodePagesEncodingProvider.Instance);
			mEncode = System.Text.Encoding.GetEncoding("Shift_JIS");
		}
		...
		public bool Open(String filename, bool force)
		{
			...
			if (force == false)
			{
				if(mEncode.GetString(mBuffer, 0x04, 30) != "フォーマット済みファイルデータ")
				{
					mBuffer = null;
					return false;
				}
		...
		{
			return address + Adventure * 0x2800 + 0x28;
		}

		private uint CalcCheckSum()
		{
			uint sum = 0;
			for (uint i = 0; i < 0x2800 - 4; i++)
			{
				sum += ReadNumber(i + 4, 1);
			}
